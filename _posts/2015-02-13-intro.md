...
第一步 ：安装\\
下载 ZooKeeper：[[http://www.apache.org/dyn/closer.cgi/zookeeper/|下载链接]]\\
解压 ZooKeeper 包
...
这里有几点注意事项需要说明：\\
1. 实体对象必须使用@Table进行注解，否则框架不会识别此对象\\
2. 实体对象需要实现java.io.Serializable接口\\
3. 实体对象必须包含一个数字类型的主键字段并且使用@PrimaryKey标注\\
4. 实体对象中至少包含一个字段\\
