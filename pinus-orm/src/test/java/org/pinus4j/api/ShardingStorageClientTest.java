...
public class ShardingStorageClientTest extends BaseTest {
    ...
    public void testFindByPkNumberClassOfT() {
        TestGlobalEntity globalEntity = storageClient.findByPk(ShardingStorageClientTest.globalPk1,
                TestGlobalEntity.class);
    ...
    public void testFindByPkListListOfQextendsNumberClassOfT() {
        List<TestGlobalEntity> globalEntities = storageClient.findByPkList(
                Arrays.asList(ShardingStorageClientTest.globalPks), TestGlobalEntity.class);
        ...
        // test cache
        globalEntities = storageClient.findByPkList(Arrays.asList(ShardingStorageClientTest.globalPks),
                TestGlobalEntity.class);
        for (int i = 0; i < globalEntities.size(); i++) {
            Assert.assertEquals(ShardingStorageClientTest.globalEntities.get(i), globalEntities.get(i));
        }

        List<TestEntity> shardingEntities = storageClient.findByPkList(
                Arrays.asList(ShardingStorageClientTest.shardingPks), TestEntity.class);
        ...
        // test cache
        shardingEntities = storageClient.findByPkList(Arrays.asList(ShardingStorageClientTest.shardingPks),
                TestEntity.class);
        for (int i = 0; i < shardingEntities.size(); i++) {
            Assert.assertEquals(ShardingStorageClientTest.shardingEntities.get(i), shardingEntities.get(i));
        }
    ...
    public void testFindByPkListListOfQextendsNumberClassOfTBoolean() {
        List<TestGlobalEntity> globalEntities = storageClient.findByPkList(
                Arrays.asList(ShardingStorageClientTest.globalPks), TestGlobalEntity.class, false);
        for (int i = 0; i < globalEntities.size(); i++) {
            Assert.assertEquals(ShardingStorageClientTest.globalEntities.get(i), globalEntities.get(i));
        }

        List<TestEntity> shardingEntities = storageClient.findByPkList(
                Arrays.asList(ShardingStorageClientTest.shardingPks), TestEntity.class, false);
        for (int i = 0; i < shardingEntities.size(); i++) {
            Assert.assertEquals(ShardingStorageClientTest.shardingEntities.get(i), shardingEntities.get(i));
        }
    }
    ...
    public void testFindBySqlSQLClassOfQ() {
        ...
        for (int i = 0; i < ShardingStorageClientTest.globalEntities.size(); i++) {
            Assert.assertEquals(ShardingStorageClientTest.globalEntities.get(i).getId(), globalEntities.get(i)
                    .get("pk"));
        }
    }
    ...
    public void testFindByPkListListOfQextendsNumberIShardingKeyOfQClassOfT() {
        List<TestEntity> shardingEntities = storageClient.findByPkList(
                Arrays.asList(ShardingStorageClientTest.shardingPks), shardingKey1, TestEntity.class);
        for (int i = 0; i < shardingEntities.size(); i++) {
            Assert.assertEquals(ShardingStorageClientTest.shardingEntities.get(i), shardingEntities.get(i));
        }
        // test cache
        shardingEntities = storageClient.findByPkList(Arrays.asList(ShardingStorageClientTest.shardingPks),
                shardingKey1, TestEntity.class);
        for (int i = 0; i < shardingEntities.size(); i++) {
            Assert.assertEquals(ShardingStorageClientTest.shardingEntities.get(i), shardingEntities.get(i));
        }
    }
    ...
    public void testFindByPkListListOfQextendsNumberIShardingKeyOfQClassOfTBoolean() {
        List<TestEntity> shardingEntities = storageClient.findByPkList(
                Arrays.asList(ShardingStorageClientTest.shardingPks), shardingKey1, TestEntity.class, false);
        for (int i = 0; i < shardingEntities.size(); i++) {
            Assert.assertEquals(ShardingStorageClientTest.shardingEntities.get(i), shardingEntities.get(i));
        }
    }
